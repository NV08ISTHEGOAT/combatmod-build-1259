...
/**
 * Click analysis. The client handles clicks once per tick, so clicks are measured in client ticks (counted from
 * ClientTickEnd packets), which removes network jitter. Swings caused by mining or using blocks are ignored.
 */
public final class AutoClickerCheck extends Check<AutoClickerCheck.State> {
    ...
    public AutoClickerCheck() {
        super("AutoClicker", Category.COMBAT, "Clicking faster or more evenly than a human can", false, 1.0, 0.0, 0.05);
        option("maxCps", 22.0);
        option("samples", 40.0);
        option("minCps", 8.0);
    ...
    public void onTickEnd(ServerPlayerEntity player, PlayerData data) {
        ...
        if (st.delays.size() >= opt("samples")) {
            ...
            if (averageCps >= opt("minCps") && deviation < opt("minDeviation")) {
                flag(player, data, "consistency",
                    String.format(Locale.ROOT, "deviation=%.3f ticks at %.1f cps", deviation, averageCps), 2.0);
            }
            st.delays.clear();
        }
    }
